...
const CategoriesPage = () => {
  return (
    <section className='flex flex-col gap-6 py-8 container w-screen h-screen'>
      <h1 className='text-3xl font-bold text-start items-center justify-start'>Categorias</h1>
      <ul key="categories-list" className="flex flex-col gap-2 w-full  h-full tablet:hidden">
          ...
          <ul className='hidden tablet:grid grid-cols-3 gap-6 w-full px-8'>
            {CategoryDesciptions.map((category) => (
              <li key={category.category}>
                <Link href={`/categorias/${category.category}`} className='underline text-xl text-gray-700 font-semibold'>
                  {category.categoryTitle}
                </Link>
              </li>
            ))}
